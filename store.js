...
let form = document.querySelector('.from-wrapper');
let saveBtn = document.querySelector('.save');
let cancelBtn = document.querySelector('.cancel');
let slidshow = document.getElementById('slidshow');

let adlete = document.getElementById('deletee');
let popup = document.querySelector('.popup');
let DeleteData =  document.getElementById('DeleteData');
let breaPopup =  document.querySelector('.break');
let cancelAll = document.querySelector('cancelAll');
...
// popup deleteData
function poppudelete(){
    popup.classList.add('action-botton');
}

DeleteData.onclick = function(){
    popup.classList.remove('action-botton');
    tabledetails()
    
}

breaPopup.onclick = function(){
    popup.classList.remove('action-botton');
    tabledetails()
}
...
if(localStorage.product != null){
    slidshow.setAttribute('src' , "click-with-arrow-115501214699dmmvlneym-removebg-preview (3).png");
...
function addToTable(){
    ...
    for( let i = 0 ; i < data.length ; i++){
        ...
        <tr>
            ...
            <td>${data[i].date}</td> 
            <td>${data[i].Promo}</td>
            <td><button onclick="UpdateData(${i})" class="adelete" id="update">Update</button></td>
            <td><button onclick="poppudelete(${i})" id="deletee">Delete</button></td>
        </tr>
        `;
    ...
    function tabledetails(){
        ...
        for(let i = 0; i < data.length ; i++){
            table = `
            <img src="icons8-verified-account-96.png" ><br><br>
                        <tr>
                        
                            <td>${ '  ' + ' ' + 'Nom : '+ data[i].nom+'<br>'}</td>
                            <td>${ '  ' + ' ' + 'marque : '+ data[i].marque +' ' + ' /' }</td>
                            <td>${ '  ' + ' ' + 'prix : '+ data[i].prix}</td>
                            <td>${ '  ' + ' ' + 'AdType : '+ data[i].AdType}</td>
                            <td>${ '  ' + ' ' + 'date : '+ data[i].date +'  ' + ' /' }</td>
                            <td>${ '  ' + ' ' + 'Promo : '+ data[i].Promo}</td>
                        </tr>
                        `;
                            setTimeout(function(){
                            ...
                            formation.style.color=' rgb(0, 199, 0)';
                            document.getElementById('formation').innerHTML = table;
            }     
        }
} 
...
let table='';
    for(let i = 0; i < data.length ; i++){
        if(data[i].nom.includes(value.toLowerCase())){
            table += `
                    <tr>
                        <td>${data[i].nom}</td>
                        <td>${data[i].marque}</td>
...
function deleteData(i){
    ...
    function deleteDataPro(){
        let table='';
        for(let i = 0; i < data.length ; i++){
           
            table = `
                        <tr>
                            <img src="icons8-remove-96.png" ><br><br>
                            <td>${ '  ' + ' ' + 'Nom : '+ data[i].nom+'<br>'}</td>
                            <td>${ '  ' + ' ' + 'marque : '+ data[i].marque +' ' + ' /' }</td>
                            <td>${ '  ' + ' ' + 'prix : '+ data[i].prix}</td>
                            <td>${ '  ' + ' ' + 'AdType : '+ data[i].AdType}</td>
